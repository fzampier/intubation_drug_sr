...
\caption*{\textbf{eFigure~\theefigure.} Subgroup analysis for etomidate vs ketamine mortality by overall risk of bias. Test for subgroup differences: $p$ = 0.94. Only one study (Punt 2014) was rated ``high risk.''}
\end{efigure}
...
\caption*{\textbf{eFigure~\theefigure.} Sensitivity analysis excluding studies with midazolam adjunct in the ketamine arm (Cinar 2011, Punt 2014 excluded; 8 studies remaining).}
\end{efigure}

\begin{efigure}[p]
\centering
\includegraphics[width=0.85\textwidth]{eFigure13_sens_no_high_rob.pdf}
\caption*{\textbf{eFigure~\theefigure.} Sensitivity analysis excluding high risk-of-bias studies (Punt 2014 excluded; 9 studies remaining).}
\end{efigure}
...
Data were extracted independently by two reviewers (or by machine extraction with full human verification against source PDFs). Below we present the mortality data summary followed by full per-study extraction forms for all 10 included studies (N = 4,752 patients).
...
\vspace{4pt}
\raggedright\footnotesize
SC = some concerns. D1 = bias arising from randomization process; D2 = bias due to deviations from intended interventions; D3 = bias due to missing outcome data; D4 = bias in measurement of the outcome; D5 = bias in selection of reported result. One study rated high risk of bias overall (Punt 2014); nine studies rated ``some concerns'' (primarily open-label or partially blinded designs). No studies were rated low risk of bias overall.
\end{table}
